...
local function fzf_files(opts)
    ...
    if opts.cwd == nil and not is_relative then
        -- If cwd is not provided and the buffer directory is not in the detected cwd,
        -- prompt for the search directory.
        vim.ui.input({
    ...
    opts.actions = {
        [search_dirs_bind] = function(_, o)
            opts.search_dirs = not o.search_dirs
            fzf_files(opts)
        end
    }

    -- If buffer is in cwd, add <C-I> keymap to filter buffer directory.
